# Streamlit 화면 구성에 필요한 ui 관련 사용자정의 함수를 모아둔 모듈임

import streamlit as st
# 웹 화면을 구성하는 데 사용하는 파이썬 웹 애플리케이션 프레임워크

def init_session_state() -> None:
    '''로그인 상태에 대한 세션 변수를 초기화하는 함수임
        Streamlit 은 버튼 클릭이나 입력 변경이 발생할 때 마다 스크립트를 위에서 부터 다시 실행함
        로그인 상태처럼 계속 유지해야 하는 값은 st.session_state 에 저장해야 함
    '''
    # logged_in 키가 세션에 없다면, 로그인 여부의 기본값을 False 로 지정
    if 'logged_in' not in st.session_state:
        st.session_state.logged_in = False   # 비로그인 상태임

    # user_id 키가 세션에 없다면, 로그인 사용자의 ID 저장공간을 생성함
    if 'user_id' not in st.session_state:
        st.session_state.user_id = None   # None 은 로그인한 사용자가 없다는 의미임 (비로그인 상태임)

    # face_score 키가 아직 세션에 없다면, 얼굴 유사도 점수를 저장할 공간을 세션에 생성함
    if 'face_score' not in st.session_state:
        st.session_state.face_score = 0.0   # 얼굴 비교 점수가 없다는 의미임 (비로그인 상태임)
...
def render_page_header() -> None:
    ...
    st.title('InsightFace 얼굴 로그인 + 고객 이탈 예측 서비스')    # Streamlit 화면에 가장 큰 제목을 출력
    # 제목 아래에 작은 안내 문구를 출력
...
def render_sidebar(default_threshold: float) -> float:
    '''사이드바 ui 를 출력하고 얼굴 인증 임계값을 반환하는 함수
        Args:
    ...
    with st.sidebar:
        ...
        if st.session_state.logged_in:          # logged_in 이 True 이면
            # 로그인한 사용자 아이디와 로그인 성공 메세지를 출력
            st.success(f'로그인 사용자 : {st.session_state.user_id}')

            # 얼굴 로그인시 계산된 유사도 점수를 소수점 3자지까지 출력
            st.info(f'얼굴 유사도 : {st.session_state.face_score: .3f}')

            # 로그아웃 버튼 생성, 버튼 클릭하면 True 가 반환될 것임
            if st.button('로그아웃'):  # 로그아웃 버튼 클릭하면
                logout()
        ...
        # 얼굴 인증 기준값을 사용자가 조정할 수 있도록 슬라이더 생성
        threshold = st.slider(
            '얼굴 인증 임계값',
            min_value=0.20,
            max_value=0.80,
...
def render_register_ui():
    '''얼굴 등록 화면을 출력하고, 사용자 입력값을 반환하는 함수
        Returns:
            ...
            user_id : 등록할 사용자 아이디
            selected_file : 카메라 촬용 이미지 또는 업로드한 이미지 파일
    '''
    ...
    # 웹캠을 사용할 수 없는 경우, 이미지 파일을 업로드할 수 있도록 파일 업로더 제공
...
def render_login_ui():
    '''얼굴 로그인 화면을 출력하고, 사용자의 입력값을 받아서 리턴하는 함수
        Returns :
            ...
            selected_file : 카메라 촬영 이미지 또는 업로드된 이미지 파일
    '''
    # 얼굴 로그인 영역의 소제목을 출력함
    st.subheader('얼굴 로그인')
    ...
    login_camera_image = st.camera_input(
        ...
        key='login_camera',  # 등록 탭의 카메라 입력과 충돌나지 않게 고유 키를 지정
    )

    # 웹캠 대신에 사용할 이미지 파일 업로드할 수 있는 파일 업로더 제공함
    login_uploaded_image = st.file_uploader(
        '로그인시 비교할 얼굴 이미지 파일 업로드',  # 파일 업로드 컴포넌트에 표시할 라벨 문구
        type=['png', 'jpg', 'jpeg'],   # 허용할 이미지 확장자 목록
        key='login_upload',   # 등록 업로드 컴포넌트와 충돌나지 않도록 고유 키 지정
    )
    ...
    # 얼굴 로그인 로직을 실행할 버튼
    clicked = st.button('얼굴 로그인 실행', type='primary')
...
def render_churn_result(result: dict) -> None:
    pass
