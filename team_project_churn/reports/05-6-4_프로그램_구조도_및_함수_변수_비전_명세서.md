...
기준 문서: [05-5](05-5_프로그램_구조도_및_함수_변수_비전_명세서.md) · [17-6-1](17-6-1_백엔드_DB_변경사항.md) · [17-6-5](17-6-5_5m_models7_호환성정합_및_개선_계획서.md) · [19 백엔드 DB 계획서](19-0_백엔드_DB_MySQL_계획서.md) · `data/processed_5m/meta_5m.json` · `preprocessing_project/*`는 백업/테스트 참고
...
```
   ...
   [dashboard_streamlit]   [models/ 파일]  *.joblib *.pth *.parquet *.npz
     (얼굴로그인 + 통합 대시보드)   (1.27M 피처/시퀀스 = DB 밖)
```
...
```
   ...
   prep.joblib/manifest        │  ▲                    얼굴로그인 + 통합 대시보드
                               │  │
                      pull로그 │  │ push결과(여유 시)
                               ▼  │
...
`face_user.user_id`가 얼굴 로그인 사용자 식별 기준(논리참조)이다. `role`은 기본 `customer`로 두며 화면 분기 기준으로 사용하지 않는다. 강제 FK는 `prediction_log.model_id→model_registry`, `recommendation.model_id→model_registry`, `retention_action_log.prediction_id→prediction_log`, `ensemble_result`는 prediction 다대다를 `ensemble_member`로 연결.
...
├── dashboard_streamlit/               단일 통합 대시보드(Streamlit)
│   ├── app.py
│   ├── pages/
│   │   ├── 01_face_login.py
│   │   └── 02_dashboard.py
...
| `dashboard_streamlit` | backend REST | 화면 | 얼굴 로그인 + 단일 통합 대시보드 |
...
API 상세는 [19 계획서](19-0_백엔드_DB_MySQL_계획서.md) §4. 배포: 백엔드(Node)는 상시 실행(별도 스레드/프로세스), MySQL은 로컬/관리형, Neon은 시뮬 로그 전용, `dashboard_streamlit`은 백엔드 REST만 호출.
...
## 12. dashboard_streamlit 얼굴 로그인 & 통합 대시보드 (v6)
...
[로그인] 얼굴 검출 -> 임베딩 정규화 -> 후보와 cosine similarity -> threshold 이상 성공
       -> face_login_log 저장 -> role 없으면 customer 기본값 -> 통합 대시보드 진입
```

`opencv_face_login` 기준: `InsightFace FaceAnalysis("buffalo_l")`, 512차원 임베딩, 코사인 유사도, 기본 임계값 `0.45`.

`face_user`와 `face_login_log`는 운영 MySQL 테이블이며, 대시보드는 직접 조회하지 않는다. 등록/로그인 요청은 백엔드 `/auth/face/register`, `/auth/face/login`으로 보내고, 백엔드가 `face_user` 조회/생성 및 `face_login_log` 저장을 수행한다. `role`은 기본 `customer`로 두며 별도 관리자/개발자 대시보드 분기에는 사용하지 않는다.
...
| 운영 KPI | 전체 위험 비율, active 모델, 고위험 고객 Top N | `/dashboard/summary`, `/predictions/top-risk` |
| 모델 진단 | 15개 시각화 후보(PR/ROC/threshold/calibration/lift/SHAP 등) | `/models/:id/charts/:chartName` |
| 비즈니스 가치 | Value at Risk, Revenue Recovery Simulation | chart API, `business_value_json` |
...
9. dashboard_streamlit이 백엔드 REST 조회 -> 단일 통합 대시보드 표시
