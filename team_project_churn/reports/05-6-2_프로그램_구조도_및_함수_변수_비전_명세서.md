...
기준 문서: [05-5](05-5_프로그램_구조도_및_함수_변수_비전_명세서.md) · [17-6-1](17-6-1_백엔드_DB_변경사항.md) · [17-6-5](17-6-5_5m_models7_호환성정합_및_개선_계획서.md) · [19 백엔드 DB 계획서](19_백엔드_DB_MySQL_계획서.md) · `preprocessing_project/v2_5m/README.md`
...
**v6-2 보강점**
- `05-5`에 있던 **학습 환경, 모델별 추가 전처리, Streamlit 대시보드 상세, Vercel Thin API, 파라미터/하이퍼파라미터, 산출물 공유, 실행 순서, 배포 옵션, 시각화 이미지 인덱스**를 v6 토폴로지에 맞춰 누락 없이 계승했다.
- v6에서 새로 생긴 백엔드·MySQL·모델 제출 계약과 충돌하지 않도록, v5의 "로컬 Streamlit 직접 추론" 표현은 **모델파트 학습/검증 + 백엔드 ingest + Streamlit 조회** 구조로 갱신했다.
...
API 상세는 [19 계획서](19_백엔드_DB_MySQL_계획서.md) §4. 배포: 백엔드(Node)는 상시 실행(별도 스레드/프로세스), MySQL은 로컬/관리형, Neon은 시뮬 로그 전용, Streamlit은 백엔드 REST만 호출.
...
추론·학습을 백엔드에 밀어 넣지 않는다. v6 기준 학습은 **모델파트/전처리 프로젝트** 책임이고, 백엔드는 학습 종료 후 생성된 산출물 패키지를 검증·등록·서빙한다.

| 단계 | 환경 | 입력 | 산출물 | v6 책임 |
| --- | --- | --- | --- | --- |
| EDA/라벨 검증 | 로컬/Colab | REES46 5개월 원본·정제 파일 | 분포표·라벨 검증 그래프 | 모델파트 |
| 피처/시퀀스 생성 | 로컬 | 정제 이벤트 | `processed_5m/*.parquet`, `*.npz`, `meta_5m.json` | 전처리 프로젝트 |
...
**공유 순서**: `processed_5m` 확정 → `models7` 학습 → `model_run_manifest.json` 작성 → `POST /models/submit` → 백엔드가 `model_registry`/평가 요약 저장 → Streamlit 대시보드 표시.
...
## 12. Streamlit 얼굴 로그인 & 대시보드 (v6)

v5에서는 Streamlit이 DB와 모델을 직접 다뤘지만, v6에서는 **Streamlit은 화면·얼굴 로그인·조회 UI**이고 데이터 접근은 백엔드 REST로 통일한다.
...
| 영역 | 내용 | 백엔드/DB 출처 |
| --- | --- | --- |
| 내 이탈 위험 | 이탈 확률·위험 등급·최근 예측 시각 | `/predictions/latest`, `prediction_log` |
...
전체 위험 비율, 위험등급 분포, 이벤트 퍼널, active 모델 성능, 모델별 PR-AUC/ROC-AUC, threshold, calibration, lift, SHAP summary, 고위험 고객 Top N, 리텐션 액션 상태를 조회한다. 화면은 Streamlit이 그리되 계산 결과는 백엔드 chart API에서 받는다.
...
## 13. Simulation Site & Thin API (v5 Vercel 파트 계승)
...
| 작업 | 산출물 | 위치 | 공유 기준 |
| --- | --- | --- | --- |
| 원본 정제 | 정제 파일·요약 | `processed_5m/`, `meta_5m.json` | 행 수/기간/라벨 검산 |
| 라벨/피처 | tabular parquet, seq npz | 파일 + DB 경로 | 10피처·라벨2종·시퀀스 shape |
| ML 학습 | model, prep artifact, metrics | `models/`, `model_registry` | `preprocessing_config` 포함 |
| DL 학습 | `.pth`, metrics, loss curve | `models/sequence/`, evaluation artifact | train/val history 포함 |
...
```text
1. MySQL 운영 DB 생성 -> backend/schema_mysql.sql 적용
2. Neon 시뮬 로그 DB 준비 -> Simulation Site /events 연결
3. REES46 5개월 전처리 -> processed_5m parquet/npz/meta 확정
...
7. Simulation Site에서 고객 행동 시뮬레이션 -> Neon sim_event_log 저장
8. 백엔드가 최근 이벤트를 읽어 feature/sequence snapshot 생성 -> prediction/ensemble/recommendation 저장
9. Streamlit 대시보드가 백엔드 REST 조회 -> 고객/관리자 화면 표시
...
| **B'. 현재 v6 확정** | **Node 백엔드 + MySQL + Neon 시뮬 + Streamlit** | **모델파트 산출물 제출, 백엔드는 ingest/서빙** | 클린아키텍처 경계 명확, 발표 대시보드 확장 쉬움 | 백엔드 상시 실행 필요 |
...
| `assets/05-5/06_streamlit_dashboard_wireframe.png` | Streamlit 와이어프레임 |
